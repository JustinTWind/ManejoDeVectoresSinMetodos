...
public class manejoDeVectores {
    ...
    public static int [] llenarVector(){
        ...
        for (int i = 0; i < n; i++) {
            vector[i] = (int) (random.nextInt(100,200));
    ...
    public static int[] binarySearch (int[] vector, int value) throws ElementNotFoundException {
        ...
        while (start <= end) {
            ...
            if (vector[position] == value) {
                ...
                return limpiarCerosVector(bubbleSorting(indexes));
    ...
    public static int[] limpiarCerosVector(int[] vector) {
        int contador = 0;
        for (int num : vector) {
            if (num != 0) contador++;
        }
